...
public class BeeCompressor {
		...
		try {
			...
			if (type.equalsIgnoreCase("jsp")||type.equalsIgnoreCase("html")||type.equalsIgnoreCase("htm")) {
				...
				while((index = jspContent.indexOf("<script",start))!=-1){					
					...
					this.compressJS(sr, sw,this.linebreakpos);					
				...
				compressJS(in, out,this.linebreakpos);
	...
	private void compressJS(Reader in, Writer out,int lineBreak) throws IOException {
		...
		try {
			...
			compressor.compress(out, lineBreak, munge, verbose,
